...
You can view the Report in Python, export the outputs (HTML, JSON, Python dictionary) or upload it to the ServeQuery platform. Check more in [output formats](/docs/library/output_formats).
...
<Tip>
  **All descriptors and parameters**. ServeQuery has multiple implemented descriptors, both deterministic and LLM-based. See a [reference table](/metrics/all_descriptors) with all descriptors and parameters.
</Tip>
...
**Custom Report with different Metrics**. Each ServeQuery Report is built from individual Metrics. For example, `TextEvals` internally uses `ValueStats` Metric for each descriptor. To customize the Report, you can reference specific descriptors and use metrics like `MeanValue`, `MaxValue`, etc:
