...
<Info>
  **This is a local example.** You will run and explore results using the open-source Python library. At the end, we’ll optionally show how to upload results to the ServeQuery Platform for easy exploration.
</Info>
...
We will focus on demonstrating **how to create and tune the LLM evaluator**, which you can then apply in different contexts, like [regression testing](/examples/LLM_regression_testing) or prompt comparison.
...
<Info>
  Run a sample notebook: [Jupyter notebook](https://github.com/servequery/community-examples/blob/main/tutorials/LLM_as_a_judge_tutorial_updated.ipynb) or [open it in Colab](https://colab.research.google.com/github/servequery/community-examples/blob/main/tutorials/LLM_as_a_judge_tutorial_updated.ipynb).
</Info>
...
from servequery.presets import TextEvals, ValueStats, ClassificationPreset
from servequery.metrics import *

from servequery.llm.templates import BinaryClassificationPromptTemplate
```
...
<Note>
  **Synthetic data**. You can also generate example inputs for your LLM app using [ServeQuery Platform](/docs/platform/datasets_generate).
</Note>

**Create an ServeQuery dataset object.** Pass the dataframe and [map the column types](/docs/library/data_definition):
...
**Get a Report.** Summarize the result by generating an ServeQuery Report.
...
To be able to easily run and compare evals, systematically track the results, and interact with your evaluation dataset, you can use the ServeQuery Cloud platform.

### Set up ServeQuery Cloud

<CloudSignup />

Import the components to connect with ServeQuery Cloud:
...
Since you already created the eval, you can simply upload it to the ServeQuery Cloud.

```python
ws.add_run(project.id, my_eval, include_data=True)
```

You can then go to the ServeQuery Cloud, open your Project and explore the Report.
