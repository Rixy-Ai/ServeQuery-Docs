...
In this tutorial, we'll demonstrate how to evaluate different aspects of Retrieval-Augmented Generation (RAG) using ServeQuery.

<Info>
We’ll demonstrate a **local open-source workflow**, viewing results as a pandas dataframe and a visual report — ideal for Jupyter or Colab. At the end, we also show how to upload results to the ServeQuery Platform. If you are in a non-interactive Python environment, choose this option.
</Info>
...
<Info>
  Run a sample notebook: [Jupyter notebook](https://github.com/servequery/community-examples/blob/main/tutorials/rag_metrics.ipynb) or [open it in Colab](https://colab.research.google.com/github/servequery/community-examples/blob/main/tutorials/rag_metrics.ipynb).
</Info>
...
* We create an [ServeQuery dataset object](/docs/library/data_definition).
...
<Note>
  **Synthetic data**. You can generate a ground truth dataset for your RAG using [ServeQuery Platform](/docs/platform/datasets_generate).
</Note>
...
**Score data**. Once you have a pandas dataframe `synthetic_df`, you create an ServeQuery dataset object and choose the selected descriptors by simply listing them.
...
You can then go to the ServeQuery Cloud, open your Project and explore the Report with scored data that's easy to interact with.
