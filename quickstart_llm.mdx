...
description: "Evaluate text outputs in under 3 minutes"
---

import CloudSignup from '/snippets/cloud_signup.mdx';
import CreateProject from '/snippets/create_project.mdx';

ServeQuery helps you evaluate LLM outputs automatically. The lets you compare prompts, models, run regression or adversarial tests with clear, repeatable checks. That means faster iterations, more confident decisions, and fewer surprises in production.

In this Quickstart, you'll try a simple eval in Python and view the results in ServeQuery Cloud. If you want to stay fully local, you can also do that - just skip a couple steps.
...
from servequery.tests import lte, gte, eq
from servequery.descriptors import LLMEval, TestSummary, DeclineLLMEval, Sentiment, TextLength, IncludesWords
...
Local Reports are great for quick experiments. To run comparisons, keep track of the results and collaborate with others, upload the results to ServeQuery Platform.

**Upload the Report to ServeQuery Cloud** together with scored data:

```python
ws.add_run(project.id, my_eval, include_data=True)
```

**Explore.** Go to [ServeQuery Cloud](https://app.servequery.cloud/), open your Project, and navigate to Reports. You will see all score summaries and can browse the data. E.g. sort to find all answers labeled as "Denials".
